...
goap_actionlist_t
goap_planner_plan(goap_worldstate_t currentWorld, goap_worldstate_t goal, goap_actionlist_t allActions) {
    ...
    while (da_count(stack) > 0) {
        ...
        for (ITERATE_ARRAY(neighbours)) {
            ...
            if (goap_worldstate_compare(newWorld, goal)) {
                ...
                printf("Reached goal!\n");
                // we're not making a new node so we have to free newWorld here
                map_deinit(&newWorld);
            } else {
